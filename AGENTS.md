...
npm run dist       # 打包安装版（NSIS）到 dist/
```

窗口顶部 **抖音优化** 菜单可启用 / 停用脚本、打开配置界面（`Ctrl+,`）、导出 / 导入配置。

> 只发布安装版（曾经也发便携版，已停掉 —— 便携版每次启动要自解压，约多花 12 秒，见
> 「[启动耗时](#启动耗时日志从哪一刻开始算)」）。
>
...
| 脚本的开关名是**接口**，不是内部细节 | 应用靠 `GM_Panel.dy-cookie-remove__ac__` 和 `__ac_signature` 这些字符串判断，而上游 bundle 我们并不控制。上游一改名就静默失效：警告不再响、一键关闭也点不动，而故障照旧。`test/harmful-settings.test.js` 会去 bundle 里核对这几个字符串 |
...
**`logs/` 也在跳过之列，理由不同**：它是**上一个安装**的诊断历史。复制过来会让新装的日志开头是
另一个版本的三千多行（`logFile` 指向 `%APPDATA%`、版本号落后两个版本），而最新的那行已经是几小时前 ——
排查时从头上读日志的人会被带偏。新装就该有新日志，旧的仍在原处。
...
**真推荐流上的端到端验证**（两个开关都开、未登录也能跑）：

| 观察 | 结果 |
| --- | --- |
| 脚本的原选择器实际命中 | `count: 1`，**其中 1 个含 `<video>`**（1426×839 的播放器层） |
| 守卫是否动作 | 日志两条「已移除会隐藏视频的样式规则」—— 关闭按钮那条**和**搜索栏那条 |
| 视频层可见性 | `display: block`、`hiddenAncestor: null` |
| 那两条规则还在不在 | style 标签文本仍在，但 `sheet.cssRules.length === 0`（被守卫删空） |
| CSS 本身合法吗 | 现场新插入同样文本 → 解析出 **1 条规则**，证明空 sheet 是守卫造成的，不是解析失败 |

最后一行是这次验证里最容易被误读的一点：**只看 `parsedRules: 0` 会以为"规则没生效、所以守卫没做事"，
实际恰好相反 —— 正是守卫把规则删空了**。要分清这两种情况，就在现场插一份同样的 CSS 看它能否解析。
日志要**在扫描之后**再读（守卫有 300ms 防抖），否则会得到"守卫没动作"的假结论。
...
日志落在**数据目录**下（安装版是 `<安装目录>\data\logs\main.log`），不与开发运行共享 ——
见「[数据目录](#数据目录打包后的版本自带数据)」。

**上限是硬的。** 2 MB 轮转、保留 2 份，也就是最多 6 MB。但轮转本身可能失败，最现实的路径是
**用户用菜单打开了日志**：Windows 拒绝重命名一个被别的进程打开的文件，于是每次写入都静默跳过轮转
（那里刻意不为了记一行日志而失败），"2 MB 上限"就不再是上限。所以文件超过上限 4 倍时，保留最新的
一部分、丢弃较早内容，并在文件里写明原因。**有界的日志带一段缺口，好过无界的日志。**
...
**首要嫌疑：「移除某些Cookie」开着。** 症状是**第一次打开正常、第二次就黑屏**，而且阶梯第 2 轮
报 `removedCookies: []` —— 找不到 `__ac_*` 可删，因为它们早就被脚本删掉了。

脚本的 `dy-cookie-remove__ac__` 开关会在**每次页面加载**时 `cookieManager.delete` 掉
`__ac_signature` / `__ac_referer` / `__ac_nonce`（`removeCookie()`，经 `Panel.execMenu` 注册，
所以每次导航都会跑）。这三个是服务器发给**这个客户端**的签名凭证；删掉之后下一次请求就成了
"服务器认得你、但你没带签名"，于是返回 **200 + 空 body** —— 正是本应用整套恢复机制要对付的那个空文档。
脚本自己的默认值是 `false`，它的说明也写着"阻止触发验证弹窗（**maybe**）"，是个猜测性的规避开关。

**用户总会拿浏览器对比**（"Edge 里刷抖音一点事没有，随便刷新都没限制"），这个对比是对的，而且恰恰
指向这个原因：浏览器不会删这些 cookie。0.2.4 之前**应用和文档都没有提过这件事**，所以它看起来
像服务器在限制用户 —— 这是它最贵的地方。

实测确认：同一个 profile、同一台机器、同一个构建，**只把这一个开关关掉就恢复正常**（日志从
"页面加载为空"变成站点自跳转 `?recommend=1` + 8 条控制台输出）。

所以现在：

- 启动时若该开关开着，日志里会有一条 warn（`lifecycle.js` 的 `warnAboutKnownHarmfulSettings`）；
- **工具 → 关闭「移除某些Cookie」** 一键关掉并广播 `gm-store-replaced`，运行中的页面立即生效；
- 规则与键名在 `storage/harmful-settings.js`（不依赖 electron，可单测）。上游若改了这个键名，
  `test/harmful-settings.test.js` 会失败 —— **静默找不到键就等于警告再也不响**。
- **不静默改用户配置**：开关是用户的，应用只提醒并提供一个一键关闭。
...
| 轮次 | 动作 | 触发前的等待 |
| --- | --- | --- |
| 1 | 清站点非 cookie 存储 | 0（立即） |
| 2 | 再加：删 `__ac_signature` / `__ac_nonce` / `__ac_referer`（**只删反爬 cookie，登录 cookie 一个不碰**） | 5s |
| 3 | 再加：清 HTTP 缓存 | 20s |
| 4+ | 重复第 3 轮 | 60s → 180s → 300s → 600s（上限） |

**第 1 轮立即、其余每轮都要等 —— 这是被一次真实故障修出来的。** 前三轮原先的等待都是 0，而每轮都以
`reloadIgnoringCache()` 收尾，也就是**不到两秒内对服务器发了 3 次请求**（实测日志：`21:44:57.021`、
`21:44:57.820`、`21:44:58.767`）。服务器有一个**会因请求频率而收紧**的状态，所以这个"激进的开场"
恰恰让它正在修的那个故障更容易持续。用户的原话是「自动等待重试 6 次也没好」。

上限从 300s 拉到 600s 是同一个理由：实测**机器静默约 5 分钟**服务器才恢复，而每 5 分钟重试一次
等于永远不给它这 5 分钟 —— **一个会阻止恢复的修复，比一个放弃并说明的修复更糟**。

> **这两段写在「移除某些Cookie」那个真因被发现之前，所以要打折看。** 当时"静默 5 分钟才恢复"的观察，
> 很可能是在那个开关开着的情况下做的 —— 恢复的原因未必是"静默"，也可能是别的东西变了。
> 间隔本身仍然保留（三秒内连发三次请求不像话，而且修复是兜底手段，等久一点几乎没代价），
> 但**"服务器会因为请求频率而收紧"这个结论目前证据不足**，不要再拿它当解释用户故障的首要理由。
> 首要理由是上面那个开关 —— 用户拿浏览器一对比就能证明服务器没在限制他。

**另外注意：第 2 轮的动作和那个有害开关是同一件事** —— 都删 `__ac_*`。区别在时机：开关是在
**页面正常时**每次加载都删，把好的请求变成坏的；阶梯只在**页面已经空了**的时候删一次，那时删掉一个
可能已经失效的签名，还有机会让服务器重新发一个挑战。所以它保留 —— 但要知道它是在"已经坏了"的
前提下做的动作，**不能理解成"删这些 cookie 是好事"**。

过了阶梯之后，日志与窗口标题改成「服务器暂时没有返回页面，正在等待重试」—— 不是本地能修的事，
就不该说成"正在自动修复"。**给用户的建议也随之明确：不要反复重启。**
...
```console
npm run dist        # 安装版 → dist/抖音 Setup x.y.z.exe（NSIS，单用户，可选安装目录）
npm run clean:stale # 清理历史遗留的构建目录
```

**只发布安装版。** 曾经同时发布便携版，已停掉：便携版每次启动都要自解压约 470 MB，实测到窗口就绪
约 15.7 秒，而安装版热启动只要 270–392 ms（数字见
「[启动耗时](#启动耗时日志从哪一刻开始算)」）。少一个产物也少一份要维护的说明。
...
（安装目录里那份 `data\` 会随卸载一起删 —— 见「[数据目录](#数据目录打包后的版本自带数据)」。）
